...
| Фаза | Що | Статус |
|------|----|--------|
| **0** | Scaffold + Prisma (PostgreSQL) + Auth + nginx + SSL | ✅ Готово |
| **1** | Календар + SSE + PostModal (3 вкладки) + QuickCreate | ✅ Готово |
| **2** | Редагування постів + генерація зображень + AI чатик | ✅ Готово |
| **3** | Категорії + Персонажі + Мережі + Розклад UI | ✅ Готово |
| **4** | Статистика + Admin панель + Mobile nav | ✅ Готово |
| **5** | Сховище (медіатека drag&drop) | ⏳ Наступне |
| **6** | Bulk операції в календарі | ⏳ Наступне |
| **7** | Запрошення по email (invitation flow) | ⏳ Наступне |
| **8** | Drag & drop перетягування постів між датами | ⏳ Наступне |
| **9** | Тижневий/денний вигляд календаря | ⏳ Наступне |

## 11. Що ще потрібно реалізувати (backlog)

### Prioritized:
1. **Медіасховище** — upload файлів на сервер, grid/list, папки, теги, прикріплення до поста
2. **Bulk операції** — checkbox hover на постах → toolbar: "Видалити вибрані", "Змінити статус", "Перенести дату"
3. **Invitation flow** — superadmin надсилає invite link на email → юзер реєструється по токену
4. **Drag&drop** постів між датами (@dnd-kit уже в залежностях)
5. **Тижневий вигляд** — compact calendar з 7-денним горизонтом, корисно для щоденного планування
6. **Копіювати пост** — дублювати існуючий пост (кнопка в footer модалки)
7. **Шаблони постів** — content-types сторінка: готові структури з промптами для AI
8. **Bulk schedule** — "Запланувати всі чернетки цього тижня" одною кнопкою

### UX покращення:
- Keyboard shortcuts (Esc закриває модалки, Ctrl+S зберігає)
- Підтвердження виходу зі сторінки з незбереженими змінами
- Undo для видалення (toast з undo кнопкою, 5 сек)
- Drag textarea вгору/вниз для зміни порядку items в carousel/thread
- Preview поста (як він виглядатиме в Threads/Instagram)
- Емодзі-picker для textarea

### Технічні:
- Rate limiting на API routes
- Optimistic updates в React Query (для швидшого UX)
- Image lazy loading в календарі
- PWA (manifest + service worker) для мобільного
