...
export type StructureSeed = {
  ...
  slideRoles?: string[]; // машинно-читана послідовність ролей слайдів (тільки для carousel-структур)
};
...
export const DEFAULT_STRUCTURES: StructureSeed[] = [
  ...
  { key: "carousel_vyznannya_do_produktu", name: "Карусель: визнання/новина → продукт (reveal-арка)", platforms: ["instagram", "linkedin"], postTypes: ["carousel"], hookTypes: ["story", "promise"], structure: "Слайд 1 = анонс-обкладинка (особисте визнання/новина: грант, нагорода, подія) з фото автора. Слайд 2 = рутина/біль, яку це визнання нарешті дозволяє вирішити (буллет-список + фото-доказ). Слайд 3 = підтвердження компетентності в темі (ми вже це робимо). Слайд 4 = «що вже працює» — нумерований список [1]-[5] поверх затемненого групового фото. Слайд 5 = розгорнута ідея/продукт суцільним абзацом + виділений блок-цитата з ключовим уточненням + фото-метафора. Слайд 6 = особисте «і саме це мене зараз…» (портрет), речення обривається і йде далі хвилястою лінією-конектором. Слайд 7 = продовження речення через лінію-конектор + «що нам потрібно зараз» (буллети-прохання: прискорити, зібрати команду) + місток до підтримки/гранту. Слайд 8 = «кому це буде корисно» — сегменти аудиторії в 2 колонки з іконками. Слайд 9 = фінальний маніфест у рамці-цитаті (жирний ключовий виділений фрагмент) + подяка за підтримку.", rules: "Коли застосовувати: є привід (грант, нагорода, визнання, помітна подія), який можна розгорнути в анонс продукту чи великої ініціативи — особиста новина плавно переростає в презентацію рішення. Дизайн: rounded pill-бейдж з назвою етапу у верх-лівому куті на кожному сюжетному слайді; хвиляста лінія-конектор — наскрізний елемент, що переносить кінець речення з одного слайду на інший (не обривай думку в межах слайду без цього прийому); нумеровані пункти в квадратних дужках [1]-[5] розкидані по затемненому груповому фото для блоку «що вже є»; полароїд-фото з рукописним підписом для емоційного акценту; фото-метафора (напр. подарунок) для абстрактної ідеї; текстова рамка (border-only box) для ключової цитати/маніфесту в фіналі; приглушена консистентна палітра (оливково-зелений/бежевий) на всю карусель; дрібний юзернейм-ватермарк по центру текстових слайдів. 8-9 слайдів; підпис допису — окремим текстом, не дублює слайди.", slideRoles: ["cover", "list", "backup_hook", "photo_numbered", "quote", "photo_portrait", "photo_portrait_cont", "audience_grid", "manifesto"] },

  // ── 5 наративних структур каруселі (синтез ринкових досліджень NotebookLM, 2026-08-31) ──
  // Кожна — готова послідовність slideRoles під конкретну ціль посту; create_post
  // бере slideRoles як каркас, коли обрано цю структуру, замість вигадувати ролі з нуля.
  { key: "carousel_aida", name: "Карусель AIDA (прогрів → конверсія)", platforms: ["instagram", "linkedin"], postTypes: ["carousel"], hookTypes: ["pain", "promise"], structure: "Attention: обкладинка-хук. Interest: 3 практичні поради з деталями. Desire: кейс клієнта + бажаний кінцевий стан. Soft CTA: інструмент/продукт як полегшення. Recap-чеклист для збереження. Action: коментар-тригер.", rules: "Коли застосовувати: навчальний контент для холодної/теплої аудиторії з м'яким переведенням у DM-воронку — золотий стандарт для коучингу/консалтингу/SaaS/EdTech.", slideRoles: ["cover", "backup_hook", "list", "photo_numbered", "feature_card", "case_study", "quote", "spec_stack", "recap_checklist", "cta_button"] },
  { key: "carousel_pas", name: "Карусель PAS (Проблема → Загострення → Рішення)", platforms: ["instagram", "linkedin"], postTypes: ["carousel"], hookTypes: ["pain", "provocation"], structure: "Problem: головний біль у заголовку. Agitate: наслідки ігнорування (текст + емоційний акцент). Solution: покроковий вихід. Порівняння «було/стало» або «правильно/неправильно». Recap. CTA з ключовим словом для DM.", rules: "Коли застосовувати: швидка конверсія через гострий біль — B2B-послуги, фінанси, юридичні/медичні ніші, будь-де є чіткий «до/після» вихід.", slideRoles: ["cover", "list", "quote", "photo_numbered", "list", "feature_card", "list", "versus_matrix", "recap_checklist", "cta_button"] },
  { key: "carousel_bab", name: "Карусель BAB (До → Після → Міст)", platforms: ["instagram", "linkedin"], postTypes: ["carousel"], hookTypes: ["story", "stat"], structure: "Before: незадовільний стан. After: різкий контраст-доказ одразу поруч. Виклики, які заважали раніше. Bridge: 3-4 інструменти/дії, що забезпечили перехід (скріншоти/кроки). Відгук клієнта або схема мосту. CTA на діагностику.", rules: "Коли застосовувати: є реальний доведений кейс трансформації — фітнес, косметологія, бізнес-коучинг, агенції. Тільки реальні цифри й скріншоти, не вигадані.", slideRoles: ["cover", "before_after", "backup_hook", "list", "photo_numbered", "interface_walkthrough", "list", "feature_card", "case_study", "cta_button"] },
  { key: "carousel_4ps", name: "Карусель 4Ps (Обіцянка → Картинка → Доказ → Заклик)", platforms: ["instagram", "linkedin"], postTypes: ["carousel"], hookTypes: ["promise", "story"], structure: "Promise: яскрава обіцянка-заголовок. Picture: ідеальний стан/свобода/результат. Proof: емпіричні дані, графіки, відгуки. Push: заклик написати слово-тригер.", rules: "Коли застосовувати: аспіраційні продажі курсів/преміум-програм/стилю життя — особисті бренди, інфобізнес, лайфстайл-коучинг.", slideRoles: ["cover", "typographic_hook", "list", "before_after", "spec_stack", "case_study", "photo_numbered", "list", "recap_checklist", "cta_button"] },
  { key: "carousel_multi_sku", name: "Карусель «Каталог» (кілька товарів)", platforms: ["instagram"], postTypes: ["carousel"], hookTypes: ["listicle", "promise"], structure: "Тематична обкладинка-категорія. Кожен наступний слайд = один товар (фото + ціна + деталі). Зведена таблиця розмірів/варіантів. CTA перейти в біо або написати кодове слово зі знижкою.", rules: "Коли застосовувати: демонстрація товарного асортименту e-commerce/роздрібного бренду — легкий естетичний перегляд колекції без перенасичення стрічки окремими постами.", slideRoles: ["cover", "product_photo_cover", "circle_photo_frame", "product_photo_cover", "circle_photo_frame", "product_photo_cover", "circle_photo_frame", "product_photo_cover", "versus_matrix", "cta_button"] },
];
...
export async function seedDefaultStructures(projectId: string): Promise<{ created: number; updated: number }> {
  ...
  for (let i = 0; i < DEFAULT_STRUCTURES.length; i++) {
    ...
    const data = {
      ...
      rules: s.rules ?? null, slideRoles: (s.slideRoles as any) ?? null, sortOrder: i,
    };
